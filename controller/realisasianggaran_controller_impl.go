...
// @Failure 400 {object} web.WebResponse "Bad Request"
// @Failure 500 {object} web.WebResponse "Internal Server Error"
// @Router /realisasianggaran/{id} [get]
...
// @Success 200 {object} web.WebResponse{data=[]web.RealisasiAnggaranResponse} "OK"
// @Failure 500 {object} web.WebResponse "Internal Server Error"
// @Router /realisasianggaran/{kode_subkegiatan}/{bulan}/{tahun} [get]
func (controller *RealisasiAnggaranControllerImpl) FindAll(c echo.Context) error {
	kodeSubkegiatan := c.Param("kode_subkegiatan")
	bulan := c.Param("bulan")
	tahun := c.Param("tahun")
	if kodeSubkegiatan == "" || bulan == "" || tahun == "" {
		return c.JSON(http.StatusBadRequest, web.WebResponse{
			Code:   http.StatusBadRequest,
			Status: "BAD_REQUEST",
		})
	}
	RealisasiAnggaranResponses, err := controller.RealisasiAnggaranService.FindAll(c.Request().Context(), kodeSubkegiatan, bulan, tahun)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, web.WebResponse{
			Code:   http.StatusInternalServerError,
			Status: "INTERNAL_SERVER_ERROR",
